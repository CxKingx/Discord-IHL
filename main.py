...
async def ban (ctx):
    AU_Server_ID = 968109880428888094
    logchannel = bot.get_channel(1001886133803823144)
    for guild in bot.guilds:
        print(guild)
        if guild.id == (AU_Server_ID):
            for member in guild.members:
                # print("kicking " + str(member))
                try:
                    # await ctx.send(str(member))
                    await logchannel.send(str(member))
                    # await ctx.guild.kick(member)

                    if (member.id == 105072406976049152):
                        await member.ban(reason="noob reason")
                except:
                    # await ctx.send("Fail to kick cuz higher hiearchy")
                    await logchannel.send("Fail to kick cuz higher hiearchy")
